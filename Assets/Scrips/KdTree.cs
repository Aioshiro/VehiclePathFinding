...
public class KdTree: IEnumerable
{
    ...
    public void AddAll(List<State> items)
    {
    ...
    public KdTree FindAll(Predicate<State> match)
    {
        var list = new KdTree(_just2D);
    ...
    /// </summary>
    /// <param name="match">lamda expression</param>
    public State Find(Predicate<State> match)
    {
        var current = _root;
        while (current != null)
        {
    ...
    /// </summary>
    /// <param name="match">lamda expression</param>
    public void RemoveAll(Predicate<State> match)
    {
        var list = new List<KdNode>(_getNodes());
    ...
    public int CountAll(Predicate<State> match)
    {
        int count = 0;
        foreach (var node in this)
            if (match(node))
    ...
    private void _add(KdNode newNode)
    {
        ...
        newNode.right = null;
        newNode.level = 0;
        var parent = _findParent(newNode.component.pos);
        ...
        var splitNew = _getSplitValue(parent.level, newNode.component.pos);
    ...
    public State FindClosest(Vector3 position)
    {
    ...
    public IEnumerable<State> FindClose(Vector3 position)
    {
    ...
    protected State _findClosest(Vector3 position, List<State> traversed = null)
    {
        if (_root == null)
            return default(State);
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            var nodeDist = _distance(position, current.component.pos);
    ...
    {
        return _getSplitValue(node.level, node.component.pos);
    }
